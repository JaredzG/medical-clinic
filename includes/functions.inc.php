...
function createPatient($conn, $fname, $mname, $lname, $ssn, $sex, $bdate, $ethnicity, $race, $streetAdd, $aptNum, $city, $state, $zip, $clinic) {
  ...
  //Insert into Doctor_Maintains_Medical_Record
  $sql9 = "INSERT INTO Doctor_Maintains_Medical_Record (pat_ID, doc_ID) VALUES (?, ?);";
...
function createDoctor($conn, $fname, $mname, $lname, $ssn, $sex, $streetAdd, $aptNum, $city, $state, $zip, $deptNum, $credentials, $clinic) {
  ...
  mysqli_stmt_execute($stmt3);
  docToOffice($conn, $_SESSION["newuserID"], $clinic, 'create');
...
function createNurse($conn, $fname, $mname, $lname, $ssn, $sex, $streetAdd, $aptNum, $city, $state, $zip, $deptNum, $registered, $clinic) {
  ...
  mysqli_stmt_execute($stmt3);
  NurseToDoctorAndOffice($conn, $_SESSION["newuserID"], $clinic, 'create');
  $_SESSION["fname"] = 'admin';
  header("location: ../index.php");
  exit();
}

function docToOffice($conn, $docUserID, $clinic, $action) {
  ...
  if ($action === 'create') {
    ...
    $sql3 = "UPDATE Doctor_Works_In_Office SET office_ID = ? WHERE doctor_ID = ?;";
    $stmt3 = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($stmt3, $sql3)) {
      header("location: ../empinfo.php?error=docoffstmtfailed");
      exit();
...
function NurseToDoctorAndOffice($conn, $nurseUserID, $clinic, $action) {
  ...
  if ($action === 'create') {
    ...
    $sql3 = "UPDATE Nurse_Works_In_Office SET office_ID = ? WHERE nurse_ID = ?;";
    $stmt3 = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($stmt3, $sql3)) {
      header("location: ../empinfo.php?error=docoffstmtfailed");
      exit();
...
/*****************MEDICINE REPORT*****************/
function viewMedicationReport($conn) {
  $sql = "SELECT Medicine.name, COUNT(Medical_Record_Contains_Medicine.pat_ID) AS Patient_Count, LEFT(AVG(Medical_Record.inch_height), 5) AS Average_Height, LEFT(AVG(Medical_Record.pound_weight), 5) AS Average_Weight, LEFT(AVG(DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),Medical_Record.b_date)), '%Y') + 0), 5) AS Average_Age, CONCAT(LEFT(COUNT(CASE Medical_Record.race WHEN 'aian' THEN 1 ELSE NULL END) / COUNT(Medical_Record_Contains_Medicine.pat_ID) * 100, 5), '%') AS American_Indian_or_Alaska_Native, CONCAT(LEFT(COUNT(CASE Medical_Record.race WHEN 'a' THEN 1 ELSE NULL END) / COUNT(Medical_Record_Contains_Medicine.pat_ID) * 100, 5), '%') AS Asian, CONCAT(LEFT(COUNT(CASE Medical_Record.race WHEN 'baf' THEN 1 ELSE NULL END) / COUNT(Medical_Record_Contains_Medicine.pat_ID) * 100, 5), '%') AS Black_or_African_American, CONCAT(LEFT(COUNT(CASE Medical_Record.race WHEN 'nhopi' THEN 1 ELSE NULL END) / COUNT(Medical_Record_Contains_Medicine.pat_ID) * 100, 5), '%') AS Native_Hawaiian_or_Other_Pacific_Islander, CONCAT(LEFT(COUNT(CASE Medical_Record.race WHEN 'w' THEN 1 ELSE NULL END) / COUNT(Medical_Record_Contains_Medicine.pat_ID) * 100, 5), '%') AS White FROM Medicine RIGHT JOIN Medical_Record_Contains_Medicine ON Medicine.med_ID = Medical_Record_Contains_Medicine.med_ID RIGHT JOIN Medical_Record ON Medical_Record_Contains_Medicine.pat_ID = Medical_Record.pat_ID GROUP BY Medicine.name;";
  $stmt = mysqli_stmt_init($conn);
  if (!mysqli_stmt_prepare($stmt, $sql)) {
    header("location: transactions.php?error=gettransactionsfailed");
    exit();
...
function getPreferredClinic($conn, $userRole, $roleID) {
  ...
  $stmt = mysqli_stmt_init($conn);
  if (!mysqli_stmt_prepare($stmt, $sql)) {
    header("location: updateinfo.php?error=getoffidfailed");
    exit();
  }
  ...
  $result = mysqli_stmt_get_result($stmt);
  $row = mysqli_fetch_assoc($result);
  $sql2 = "SELECT address_ID FROM Office WHERE office_ID = ?;";
  $stmt2 = mysqli_stmt_init($conn);
  if (!mysqli_stmt_prepare($stmt2, $sql2)) {
