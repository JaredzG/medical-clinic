...
function viewMedicationReport($conn) {
  $sql = "SELECT Medicine.name, COUNT(Medical_Record_Contains_Medicine.pat_ID) AS Patient_Count, AVG(Medical_Record.inch_height) AS Average_Height, AVG(Medical_Record.pound_weight) AS Average_Weight, AVG(DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(),Medical_Record.b_date)), '%Y') + 0) AS Average_Age, CONCAT(COUNT(CASE Medical_Record.race WHEN 'aian' THEN 1 ELSE NULL END) / COUNT(Medical_Record_Contains_Medicine.pat_ID) * 100, '%') AS American_Indian_or_Alaska_Native, CONCAT(COUNT(CASE Medical_Record.race WHEN 'a' THEN 1 ELSE NULL END) / COUNT(Medical_Record_Contains_Medicine.pat_ID) * 100, '%') AS Asian, CONCAT(COUNT(CASE Medical_Record.race WHEN 'baf' THEN 1 ELSE NULL END) / COUNT(Medical_Record_Contains_Medicine.pat_ID) * 100, '%') AS Black_or_African_American, CONCAT(COUNT(CASE Medical_Record.race WHEN 'nhopi' THEN 1 ELSE NULL END) / COUNT(Medical_Record_Contains_Medicine.pat_ID) * 100, '%') AS Native_Hawaiian_or_Other_Pacific_Islander, CONCAT(COUNT(CASE Medical_Record.race WHEN 'w' THEN 1 ELSE NULL END) / COUNT(Medical_Record_Contains_Medicine.pat_ID) * 100, '%') AS White FROM Medicine RIGHT JOIN Medical_Record_Contains_Medicine ON Medicine.med_ID = Medical_Record_Contains_Medicine.med_ID RIGHT JOIN Medical_Record ON Medical_Record_Contains_Medicine.pat_ID = Medical_Record.pat_ID GROUP BY Medicine.name;";
  $stmt = mysqli_stmt_init($conn);
  if (!mysqli_stmt_prepare($stmt, $sql)) {
    header("location: transactions.php?error=gettransactionsfailed");
    exit();
